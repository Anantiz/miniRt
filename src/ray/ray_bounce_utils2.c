...
bool	ray_fract(t_collision *col, t_vector *ray, float indice_refract, \
	t_ray *refract)
{
	float		cosi;
	float		sin2t;
	...
	refract->dir = refract_calculation(vec_mult(indice_refract * cosi \
	- sqrt(1.0 - sin2t), &col->norm), indice_refract, ray);
	return (true);
}

bool	reflect_ray(int depth, t_ray *ray_flection, \
	t_collision *col, t_ray *ray)
{
	if (depth > 0)
	{
...
void	traceray_init(t_ray **ray_fraction, t_ray **ray_flection, \
	t_rgb *colorReflect, t_rgb *colorRefract)
{
